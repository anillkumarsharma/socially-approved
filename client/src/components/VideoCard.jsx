...
export default function VideoCard({ video, onClick }) {
  ...
  return (
    <button
      ...
      className="group relative shrink-0 w-[280px] h-[460px] rounded-3xl overflow-hidden bg-neutral-200 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
    >
      ...
      <div className="absolute top-3 left-3 flex items-center gap-2 text-white">
        ...
        />
        <span className="text-sm font-semibold drop-shadow">{video.creator}</span>
      </div>
    </button>
  );
}
